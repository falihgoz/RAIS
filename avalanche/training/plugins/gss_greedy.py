...
class GSS_greedyPlugin(SupervisedPlugin):
    def __init__(self, mem_size=200, mem_strength=5, input_size=[]):
        ...
        self.ext_mem_list_x = torch.zeros(mem_size, *input_size, dtype=torch.float32)
        self.ext_mem_list_y = torch.zeros(mem_size, dtype=torch.long)
        self.ext_mem_list_current_index = 0
        self.buffer_score = torch.zeros(mem_size, dtype=torch.float32)

    def before_training(self, strategy: "SupervisedTemplate", **kwargs):
        self.device = strategy.device
        self.ext_mem_list_x = self.ext_mem_list_x.to(self.device)
        self.ext_mem_list_y = self.ext_mem_list_y.to(self.device)
        self.buffer_score = self.buffer_score.to(self.device)

    def cosine_similarity(self, x1, x2=None, eps=1e-8):
        x2 = x1 if x2 is None else x2
        w1 = torch.norm(x1, p=2, dim=1, keepdim=True)
        w2 = w1 if x2 is x1 else torch.norm(x2, p=2, dim=1, keepdim=True)
        return torch.mm(x1, x2.t()) / torch.clamp(w1 * w2.t(), min=eps)

    def get_grad_vector(self, parameters, grad_dims):
        grads = torch.zeros(sum(grad_dims), device=self.device)
        offset = 0
        for param in parameters():
            if param.grad is not None:
                grad_len = param.grad.data.numel()
                grads[offset:offset + grad_len].copy_(param.grad.data.view(-1))
                offset += grad_len
        return grads
    ...
    def get_batch_sim(self, strategy, grad_dims, batch_x, batch_y):
        ...
        return torch.max(self.cosine_similarity(mem_grads, batch_grad)), mem_grads

    def get_rand_mem_grads(self, strategy, grad_dims, gss_batch_size):
        temp_gss_batch_size = min(gss_batch_size, self.ext_mem_list_current_index)
        num_mem_subs = min(self.mem_strength, self.ext_mem_list_current_index // gss_batch_size)
        mem_grads = torch.zeros(num_mem_subs, sum(grad_dims), device=self.device)

        shuffled_inds = torch.randperm(self.ext_mem_list_current_index, device=self.device)
        for i in range(num_mem_subs):
            random_batch_inds = shuffled_inds[i * temp_gss_batch_size:(i + 1) * temp_gss_batch_size]
            batch_x = self.ext_mem_list_x[random_batch_inds]
            batch_y = self.ext_mem_list_y[random_batch_inds]
            strategy.model.zero_grad()
            loss = strategy._criterion(strategy.model(batch_x), batch_y)
            loss.backward()
            mem_grads[i].copy_(self.get_grad_vector(strategy.model.parameters, grad_dims))
    ...
    def get_each_batch_sample_sim(self, strategy, grad_dims, mem_grads, batch_x, batch_y):
        ...
        for i, (x, y) in enumerate(zip(batch_x, batch_y)):
            strategy.model.zero_grad()
            loss = strategy._criterion(strategy.model(x.unsqueeze(0)), y.unsqueeze(0))
            loss.backward()
            this_grad = self.get_grad_vector(strategy.model.parameters, grad_dims).unsqueeze(0)
            cosine_sim[i] = torch.max(self.cosine_similarity(mem_grads, this_grad))
        return cosine_sim
    ...
    def before_training_exp(self, strategy, num_workers=0, shuffle=True, **kwargs):
        ...
        memory_dataset = _make_taskaware_classification_dataset(
            list(zip(self.ext_mem_list_x[:self.ext_mem_list_current_index].cpu(),
                     self.ext_mem_list_y[:self.ext_mem_list_current_index].cpu())),
            targets=self.ext_mem_list_y[:self.ext_mem_list_current_index].cpu().tolist()
        )
    ...
    def after_forward(self, strategy, **kwargs):
        ...
        grad_dims = [param.numel() for param in strategy.model.parameters()]

        if self.ext_mem_list_current_index >= self.ext_mem_list_x.size(0):
            batch_sim, mem_grads = self.get_batch_sim(strategy, grad_dims, strategy.mb_x, strategy.mb_y)
            if batch_sim < 0:
                buffer_score = self.buffer_score[:self.ext_mem_list_current_index]
                buffer_sim = (buffer_score - buffer_score.min()) / (buffer_score.max() - buffer_score.min() + 0.01)
                index = torch.multinomial(buffer_sim, strategy.mb_x.size(0), replacement=False).to(self.device)
                ...
                sub_index = outcome.squeeze(1).bool()
                self.ext_mem_list_x[index[sub_index]] = strategy.mb_x[sub_index]
                self.ext_mem_list_y[index[sub_index]] = strategy.mb_y[sub_index]
                self.buffer_score[index[sub_index]] = batch_item_sim[sub_index]
        else:
            offset = min(self.ext_mem_list_x.size(0) - self.ext_mem_list_current_index, strategy.mb_x.size(0))
            updated_mb_x = strategy.mb_x[:offset]
            updated_mb_y = strategy.mb_y[:offset]
            if self.ext_mem_list_current_index == 0:
                batch_sample_memory_cos = torch.full((updated_mb_x.size(0),), 0.1, device=self.device)
            else:
                mem_grads = self.get_rand_mem_grads(strategy, grad_dims, len(strategy.mb_x))
                batch_sample_memory_cos = self.get_each_batch_sample_sim(strategy, grad_dims, mem_grads, updated_mb_x, updated_mb_y)

            curr_idx = self.ext_mem_list_current_index
            self.ext_mem_list_x[curr_idx:curr_idx + offset].copy_(updated_mb_x)
            self.ext_mem_list_y[curr_idx:curr_idx + offset].copy_(updated_mb_y)
            self.buffer_score[curr_idx:curr_idx + offset].copy_(batch_sample_memory_cos)
            self.ext_mem_list_current_index += offset
