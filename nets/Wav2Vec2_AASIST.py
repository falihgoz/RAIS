...
class Wav2Vec2_finetune(nn.Module):
    def __init__(self):
        super(Wav2Vec2_finetune, self).__init__()
        self.wav2vec2_model_name = "facebook/wav2vec2-xls-r-300m"
    ...
    def forward(self, input_values):
        ...
        x = self.wav2vec2(input_values).last_hidden_state
        x = self.LL(x)
...
# ---------AASIST back-end------------------------#
""" Jung, J. W., Heo, H. S., Tak, H., Shim, H. J., Chung, J. S., Lee, B. J., ... & Evans, N. (2022, May). 
Aasist: Audio anti-spoofing using integrated spectro-temporal graph attention networks. 
In ICASSP 2022-2022 IEEE international conference on acoustics, speech and signal processing (ICASSP) (pp. 6367-6371). IEEE."""
...
class AASIST(nn.Module):
    def __init__(self, args=None):
        ...
        self.visual_representation = Wav2Vec2_finetune()
        ...
        # self.out_layer = nn.Linear(5 * gat_dims[1], 2) # ori
        self.out_layer = nn.Linear(5 * gat_dims[1], 80)

    def forward(self, x, Freq_aug=False):
        # --------generating visual representation--------
        x = self.visual_representation(x)

        # --------starting audio deepfake detection network--------
        # x = x.unsqueeze(1)
